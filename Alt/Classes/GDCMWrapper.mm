...
//  alt1
//
//  Class for displaying dicom images. This class uses the GDCM library
//
...
-(UIImage*) imageWithWindowWL:(NSString*) fileName :(NSInteger) wc :(NSInteger) ww {
	...
	const char *file = [fileName cStringUsingEncoding:[NSString defaultCStringEncoding]];
	...
	_retImage = WindowWL(gfile, gimage, buffer, wc, ww);
...
/**
 * Method used to return the an image with the default window width and level.
 */
...
/**
 * Method used to return the image with a specified window level and width.
 */

UIImage* WindowWL(gdcm::File const &gfile, gdcm::Image const & gimage, char *buffer, double wc, double ww)
{
	const unsigned int* dimension = gimage.GetDimensions();
	...
	else if( gimage.GetPhotometricInterpretation() == gdcm::PhotometricInterpretation::MONOCHROME2 )
    {
		...
		else if( gimage.GetPixelFormat() == gdcm::PixelFormat::UINT16 )
		{
			...
			double low = ((double)wc - 0.5 - ((double)ww-1)/2);
			double high = ((double)wc - 0.5 + ((double)ww-1)/2);
			...
			for(unsigned int i = 0; i < dimX*dimY; i++)
			{
				...
				if (val <= low) {
					...
					*pubuffer++ = 255;
					...
					unsigned short pixVal = ((val-((double)wc-0.5))/((double)ww-1)+0.5)*(255);
					*pubuffer++ = pixVal;
				}
